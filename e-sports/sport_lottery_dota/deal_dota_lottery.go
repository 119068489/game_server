...
func DealESportDOTALottery(uniqueGameId int64) {

	startStr := fmt.Sprintf("======DealESportDOTALottery中比赛uniqueGameId:%v的dota开奖  开始==========", uniqueGameId)
	...
	if errGame != nil && errGame != mgo.ErrNotFound {
		...
		s := fmt.Sprintf("=====dota开奖取得比赛表数据库错误:errGame错误:%v=====,查询条件为uniqueGameId:%v,release_flag:%v",
			errGame, uniqueGameId, for_game.GAME_RELEASE_FLAG_2)
		logs.Error(s)
		for_game.WriteFile("dota_sport_lottery.log", s)
		return
	}

	if errGame == mgo.ErrNotFound {
		s := fmt.Sprintf("===========dota开奖未取得该比赛的发布的数据、清key和设置开奖状态后  直接返回==========,查询条件为uniqueGameId:%v,release_flag:%v",
			uniqueGameId, for_game.GAME_RELEASE_FLAG_2)
		for_game.WriteFile("dota_sport_lottery.log", s)
		return
	}
	...
	if errColBet != nil && errColBet != mgo.ErrNotFound {
		...
		s := fmt.Sprintf("=======dota开奖取得投注记录数据库错误:errColBet错误:%v========,查询条件为uniqueGameId:%v,BetResult:%v",
	...
	if errColBet == mgo.ErrNotFound || nil == dbBetList || len(dbBetList) <= 0 {
		...
		s := fmt.Sprintf("=======dota开奖未取得未结算的投注记录的数据、清key和设置开奖状态后  直接返回=======,查询条件为uniqueGameId:%v,BetResult:%v",
	...
	if errColGuess != nil && errColGuess != mgo.ErrNotFound {
		...
		s := fmt.Sprintf("=======dota开奖取得赔率数据库错误:errColGuess错误:%v=======,查询条件为app_label_id:%v,game_id:%v,api_origin:%v",
	...
	if errColGuess == mgo.ErrNotFound || nil == dbGuessList || len(dbGuessList) <= 0 {
		...
		s := fmt.Sprintf("======dota开奖未取得赔率数据、清key和设置开奖状态后  直接返回======,查询条件为app_label_id:%v,game_id:%v,api_origin:%v",
			...
			if nil != betNumObj {
				...
				if betNumObj.GetStatus() == for_game.GAME_GUESS_ITEM_STATUS_0 {
					...
					if betNumObj.GetWin() == for_game.GAME_GUESS_ITEM_WIN_1 {
						...
						if nil != amountErr {
							s := fmt.Sprintf("=======DOTA开奖投计算达成时、转换用户投注额出错=========,参数为==UniqueGameId:%v,MornRollGuessFlag:%v,BetNum:%v,BetAmount:%v",
						...
						if nil != oddsErr {
							s := fmt.Sprintf("=======DOTA开奖投计算达成时、转换用户投赔率出错=========,参数为==UniqueGameId:%v,MornRollGuessFlag:%v,BetNum:%v,Odds:%v",
								betValue.GetUniqueGameId(),
								betValue.GetMornRollGuessFlag(),
								betValue.GetBetNum(),
								betValue.GetOdds())
				...
				s := fmt.Sprintf("=======dota开奖投注项不存在=========,参数为==投注OrderId:%v,投注项bet_num:%v,比赛唯一uniqueGameId:%v,动态盘口唯一UniqueGameGuessId:%v",
		...
		if nil != playerMapMsgs && len(playerMapMsgs) > 0 {
			for key, value := range playerMapMsgs {
				...
				if notifyRst.GetCode() != for_game.C_OPT_SUCCESS {
					s := fmt.Sprintf("=======dota开奖批量发送给用户消息失败,直接返回=======")
					for_game.WriteFile("dota_sport_lottery.log", s)
					return
				}
			}
	...
	endStr := fmt.Sprintf("======DealESportDOTALottery中比赛uniqueGameId:%v的dota开奖正常  结束==========", uniqueGameId)
...
func DealGameInvalid(betValue *share_message.TableESPortsGuessBetRecord, uniqueGameId int64, flag int32, reasonDetail string,
	...
	if errUpd != nil && errUpd != mgo.ErrNotFound {
		s := fmt.Sprintf("=====dota开奖设置===投注无效数据出错===,errUpd错误:%v==更新条件为OrderId:%v,BetResult:%v",
	...
	if errUpd == mgo.ErrNotFound {
		...
		s := fmt.Sprintf("======dota开奖设置===投注无效数据没有数据========更新条件为OrderId:%v,BetResult:%v",
	...
	if nil != settlement && settlement.GetCode() != for_game.C_OPT_SUCCESS {
		var s string
		if for_game.GAME_GUESS_BET_DISABLE_CODE_1 == flag {
			s = fmt.Sprintf("dota开奖返回比赛异常无效金额失败,==参数==投注OrderId:%v,投注项bet_num:%v,返回用户playId:%v,返回比赛异常无效金额amount:%v,比赛唯一uniqueGameId:%v,动态盘口唯一UniqueGameGuessId:%v",
			...
			s = fmt.Sprintf("dota开奖返回时段无效金额失败,==参数==投注OrderId:%v,投注项bet_num:%v,返回用户playId:%v,返回时段无效金额amount:%v,比赛唯一uniqueGameId:%v,动态盘口唯一UniqueGameGuessId:%v",
				betValue.GetOrderId(), betValue.GetBetNum(), betValue.GetPlayInfo().GetPlayId(), betValue.GetDisableAmount(), uniqueGameId, betValue.GetUniqueGameGuessId())
		}
		logs.Error(s)
		for_game.WriteFile("dota_sport_lottery.log", s)
...
//投注项达成
func DealGuessBetSuccess(betValue *share_message.TableESPortsGuessBetRecord, uniqueGameId int64, successAmount int64,
	playerMapMsgs map[int64][]*share_message.TableESPortsGameOrderSysMsg) *client_hall.ESportCommonResult {
	...
	if errUpd != nil && errUpd != mgo.ErrNotFound {
		s := fmt.Sprintf("dota开奖设置===投注成功数据出错===,errUpd错误:%v=====更新条件为OrderId:%v,BetResult:%v",
			errUpd, betValue.GetOrderId(), for_game.GAME_GUESS_BET_RESULT_1)
		logs.Error(s)
		for_game.WriteFile("dota_sport_lottery.log", s)
	...
	if errUpd == mgo.ErrNotFound {
		s := fmt.Sprintf("dota开奖设置===投注成功没有数据========更新条件为OrderId:%v,BetResult:%v",
			betValue.GetOrderId(), for_game.GAME_GUESS_BET_RESULT_1)
		for_game.WriteFile("dota_sport_lottery.log", s)
	...
	if nil != settlement && settlement.GetCode() != for_game.C_OPT_SUCCESS {
		s := fmt.Sprintf("dota开奖返回成功金额失败,==参数==投注OrderId:%v,投注项bet_num:%v,返回用户playId:%v,返回成功金额settlementAmount:%v,比赛唯一uniqueGameId:%v,动态盘口唯一UniqueGameGuessId:%v",
...
func DealGuessBetFail(betValue *share_message.TableESPortsGuessBetRecord, playerMapMsgs map[int64][]*share_message.TableESPortsGameOrderSysMsg) *client_hall.ESportCommonResult {
	...
	if errUpd != nil && errUpd != mgo.ErrNotFound {
		s := fmt.Sprintf("dota开奖设置===投注失败数据出错===,errUpd错误:%v====更新条件为OrderId:%v,BetResult:%v",
			errUpd, betValue.GetOrderId(), for_game.GAME_GUESS_BET_RESULT_1)
		logs.Error(s)
	...
	if errUpd == mgo.ErrNotFound {
		s := fmt.Sprintf("dota开奖设置===投注失败没有数据======更新条件为OrderId:%v,BetResult:%v",
			betValue.GetOrderId(), for_game.GAME_GUESS_BET_RESULT_1)
...
//初始化处理开奖
func InitDealDOTALottery() {

	startStr := fmt.Sprintf("========InitDealDOTALottery初始化处理DOTA 定时发送开奖  开始==========")
	for_game.WriteFile("dota_sport_lottery.log", startStr)
	...
	errQuery := col.Find(bson.M{"game_status": for_game.GAME_STATUS_2,
		...
		"app_label_id": for_game.ESPORTS_LABEL_DOTA2}).All(&sportGameLists)

	if errQuery != nil && errQuery != mgo.ErrNotFound {
		s := fmt.Sprintf("======DOTA初始化开奖查询步骤2失败=====errQuery错误:%v,查询条件为game_status:%v,===is_lottery:%v,===app_label_id:%v",
			errQuery, for_game.GAME_STATUS_2, for_game.GAME_IS_LOTTERY_0, for_game.ESPORTS_LABEL_DOTA2)
	...
	if errQuery == mgo.ErrNotFound || nil == sportGameLists || len(sportGameLists) <= 0 {
		s := fmt.Sprintf("======DOTA初始化开奖查询步骤2未查询到数据、直接返回=====,查询条件为game_status:%v,===is_lottery:%v,===app_label_id:%v",
			for_game.GAME_STATUS_2, for_game.GAME_IS_LOTTERY_0, for_game.ESPORTS_LABEL_DOTA2)
	...
	if errQuery == nil && nil != sportGameLists && len(sportGameLists) > 0 {
		...
		if nil != sportGameLists && len(sportGameLists) > 0 {
			...
			for i := 0; i < len(sportGameLists); i++ {
				...
				if DOTASysMsgTimeMgr.GetTimerById(gameId) != nil {
					DOTASysMsgTimeMgr.DelTimerList(gameId)
				}

				triggerTime := time.Duration(delayLotteryTime) * time.Second
				timer := easygo.AfterFunc(triggerTime, func() {
					DealESportDOTALottery(gameId)
				})
				DOTASysMsgTimeMgr.AddTimerList(gameId, timer)
			}
		}
	}
	endStr := fmt.Sprintf("========InitDealDOTALottery初始化处理DOTA 定时发送开奖  结束==========")
	for_game.WriteFile("dota_sport_lottery.log", endStr)
}
