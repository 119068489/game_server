...
func SendToIdelOtherServer(t int32, methodName string, msg easygo.IMessage) (easygo.IMessage, *base.Fail) {
	...
	return nil, easygo.NewFailMsg("找不到电竞对接三方接口服务器")
}
...
//取得野子科技GET请求签名
func GetYeZiApiSign(param url.Values) string {

	if nil == param {
		return ""
	}
	//如果参数中有中文参数,这个方法会进行URLEncode
	sortParams := param.Encode()
	sign := for_game.Md5(sortParams)

	return sign
}

// get 网络请求
func Get(apiURL string, params url.Values) (rs string, err error) {
	var Url *url.URL
