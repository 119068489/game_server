...
const SERVER_NAME = "sport_lottery_lol"
...
var LOLSysMsgTimeMgr = NewTimer() //LOL定时发布管理器
...
func Initialize() {
	...
	//设置服务器为存储服务器
	if for_game.GetCurrentSaveServerSid(PServerInfo.GetSid(), for_game.REDIS_SAVE_ESPORTS_LOL_SID) == 0 {
		for_game.SetCurrentSaveServerSid(PServerInfo.GetSid(), for_game.REDIS_SAVE_ESPORTS_LOL_SID)
	}
