...
DIR_LIST = ["hall","login","backstage","shop","statistics","square"]
DIR_NAME = "linux_server_" + NOW_TEXT# 临时打包目录，打包完后删除
EXECUTE_NAME = "chat_server" # 打包出来的可执行文件名
...
def deal_yaml_json_py_etc(dir_name, dir_list, is_full, group, protocol, host,center,db):       # 打包linux版本文件夹
    if not is_full:
        return
    os.chdir("execute")
    ...
    content = template.TEMPLATE_SHARE.format(group=group,center=center,db=db)
    ...
    for dir in dir_list:     #把配置文件复制到各个文件夹下
        ...
        if dir == "hall":
            ...
            with open("./config_hall.yaml", "w", encoding="utf-8") as f:
                f.write(content)
            shutil.copy("../../000000000103285.pfx", "./")
            shutil.copy("../../GHT_Root.crt", "./")
            shutil.copy("../../TESTUSER.pfx", "./")
            shutil.copy("../../TESTUSER.cer", "./")
        elif dir == "login":
            ...
            content = template.TEMPLATE_SQUARE.format(group=group, host=host)
            with open("./config_square.yaml", "w", encoding="utf-8") as f:
                f.write(content)
        else:
...
def main():
    ...
    if is_full:
        ...
        center = input("请输入ETCD服务器中心的IP(直接回车则是 {}): ".format(DEFAULT_CENTER))
        if center == "":
        ...
        if db == "":
            db = DEFAULT_DB
        while True:
            is_all = input("打包服务器all表示全部[login 、hall、backstage、shop、statistics、square]其中一个): ")
    ...
    if is_full:
        ...
        if is_all =="all":
            ...
            server_list=[is_all]
        deal_yaml_json_py_etc(DIR_NAME, server_list, is_full, group, protocol, host,center,db)
